...
public class AddSessionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        SessionFunction sessionFunc = new SessionFunction();

        boolean status = sessionFunc.AddSession(Session);
        
        
        if (status) {
            response.sendRedirect("./ReceptionistWeb/DoctorManagement/SessionList.jsp");

        } else {
            out.println("<script type=\"text/javascript\">");
            out.println("alert('Server Error! COuld not Add Session!');");
            out.println("</script>");
        }
